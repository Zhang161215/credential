...
export async function GET(
  ...
  const tx = db
    .prepare(
      `SELECT id, related_credential_id, related_credential_ids
       ...
       WHERE id = ? AND user_id = ? AND type = 'redeem'`
    )
  ...
  // Parse credential IDs from JSON array or fall back to single ID
